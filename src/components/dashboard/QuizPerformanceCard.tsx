...
export function QuizPerformanceCard({
  ...
  return (
    <Card className="group relative overflow-hidden border-border/60 bg-white/60 dark:bg-background/40 backdrop-blur-md transition-all hover:border-primary/20 hover:shadow-xl hover:shadow-primary/5 dark:hover:shadow-primary/5 shadow-sm">
      {/* Subtle Background Gradient */}
      <div 
        className={cn(
          "absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100 bg-gradient-to-br",
          isPass ? "from-emerald-500/10 via-transparent to-transparent" : "from-rose-500/10 via-transparent to-transparent"
        )} 
      />
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-bold tracking-tight">{displayPercentage}%</span>
          ...
          <div className="text-right">
            <div className="text-xs text-muted-foreground uppercase tracking-wider mb-1 flex items-center justify-end gap-1.5">
              <Target className={cn("w-3.5 h-3.5", isPass ? "text-emerald-500" : "text-rose-500")} />
              Latest
            </div>
            <div className="flex items-baseline justify-end gap-1">
               <span className={cn("text-lg font-semibold", latestScore < 0 && "text-rose-500")}>{latestScore}</span>
               <span className="text-xs text-muted-foreground">/ {maxScore} pts</span>
            </div>
          </div>
        </div>
